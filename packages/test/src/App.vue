<template>
  <div class="flex bg-background">
    <!-- 左侧导航 -->
    <aside
      class="w-72 p-4 border-r border-border fixed left-0 top-0 bottom-0 z-10"
    >
      <nav class="flex flex-col gap-6 overflow-y-auto h-screen">
        <div v-for="g in filteredTabs" :key="g.group">
    ...
    <!-- 右侧展示区 -->
    <main class="flex-1 overflow-auto relative pl-72">
      <div
        class="p-4 border-b h-16 fixed bg-background border-border flex justify-between items-center top-0 left-72 right-0 z-10"
      >
...
const filteredTabs: FilteredTabs[] = Object.entries(grouped).map(
