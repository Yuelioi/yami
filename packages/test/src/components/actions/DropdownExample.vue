  ...
  <DocsContainer>
      ...
      <div class="grid gap-4 p-6 bg-card">
        <details class="dropdown w-36">
          ...
          <div class="dropdown-content p-2 bg-white border rounded shadow-md">
            <a href="#" class="block p-1 hover:bg-gray-100">菜单项 1</a>
            <a href="#" class="block p-1 hover:bg-gray-100">菜单项 2</a>
            <a href="#" class="block p-1 hover:bg-gray-100">菜单项 3</a>
          </div>
        </details>
      </div>
    </DocsSection>
    ...
    <DocsSection title="拓展 使用 Vue 指令"
      ><p class="text-sm text-destructive mb-4 w-full">
        注意：原生 &lt;details&gt;
        元素点击外部**不会**关闭。要实现外部点击关闭，请使用v-click-modal-outside或
        div+v-if/v-show 结构。
      </p>
      <div></div>

      <details class="dropdown w-96" v-click-modal-outside>
        <summary class="btn btn-primary w-64">
          打开操作菜单 (with v-click-modal-outside)
        </summary>

        <div class="dropdown-content p-2 w-48">
          <div class="p-1">
            <button
              class="btn btn-ghost w-full text-foreground text-sm justify-start"
            >
...
const apiClasses = [
  ...
  },
  {
    category: "方向修饰",
    className:
      "dropdown-bottom / dropdown-top / dropdown-start / dropdown-end / dropdown-center",
