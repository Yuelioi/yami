  ...
  <DocsContainer>
    ...
    <DocsSection title="基础用法" class="flex-col">
      <div class="flex flex-wrap gap-4">
        <button class="btn btn-primary" @click="showModal('base')">基础结构 (MD 尺寸)</button>
        <button class="btn btn-secondary" @click="showModal('sm')">Small (sm)</button>
        <button class="btn btn-secondary" @click="showModal('lg')">Large (lg)</button>
        <button class="btn btn-secondary" @click="showModal('xl')">Extra Large (xl)</button>
        <button class="btn btn-secondary" @click="showModal('full')">Full Width (full)</button>
        <button class="btn btn-accent" @click="showModal('blur')">带背景模糊 (modal-blur)</button>
      </div>

      <button class="btn modal-control" onclick="my_modal_1.showModal()">open modal</button>
        ...
        <div class="modal-content modal-content-md">
          ...
          <div class="modal-body">
            ...
            <p class="mt-2 text-sm text-muted-foreground">
              此示例演示了 **modal-header**, **modal-title**, **modal-close** 和 **modal-body**
              类名。
            </p>
            <p class="mt-4">滚动测试:</p>
          ...
          <div class="modal-footer">
            <button class="btn btn-ghost" @click="closeModal('base')">取消</button>
      ...
      >
        <div :class="['modal-content', `modal-content-${size}`]">
          ...
          <div class="modal-body">
            <p>这个对话框使用了 `modal-content-{{ size }}` 类名。</p>
            <p v-if="size === 'full'" class="text-destructive mt-2">
              注意：'full' 尺寸接近视口宽度。
            </p>
          </div>
          <div class="modal-footer">
            <button class="btn btn-primary" @click="closeModal(size)">关闭</button>
          </div>
        </div>
      </dialog>
      ...
      <dialog ref="blurModal" class="modal modal-blur">
        <div class="modal-content modal-content-md">
          ...
          <div class="modal-footer">
            <button class="btn btn-primary" @click="closeModal('blur')">关闭</button>
          </div>
        </div>
      </dialog>
    </DocsSection>
    ...
    <section class="bg-card p-6 rounded-lg shadow-sm border border-border">
      ...
      <dialog ref="bounceModal" class="modal modal-bounce" v-click-modal-outside="">
        <div class="modal-content modal-content-sm">
          ...
          <div class="modal-footer">
            <button class="btn btn-primary" @click="closeModal('bounce')">关闭</button>
          </div>
        </div>
      </dialog>
    </section>
...
const apiClasses = [
  ...
  {
    category: 'Body 尺寸变体',
    className: 'modal-content-sm / md / lg / xl / full',
